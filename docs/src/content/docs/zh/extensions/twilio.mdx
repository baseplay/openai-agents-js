...
2. **设置一个可接收 Twilio 事件的 WebSocket 服务器。**
   ...
   如果你在本地开发，需要配置本地隧道（例如 [`ngrok`](https://ngrok.io/) 或 [Cloudflare Tunnel](https://developers.cloudflare.com/pages/how-to/preview-with-cloudflare-tunnel/)）以便 Twilio 可以访问你的本地服务器。你可以使用 `TwilioRealtimeTransportLayer` 来连接 Twilio。
